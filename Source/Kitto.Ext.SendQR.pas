...
uses
  SysUtils, Ext, ExtForm, ExtPascal,
  Kitto.Ext.Base, EF.Tree, Kitto.Ext.BorderPanel;

type
  TKExtSendQRFormPanel = class(TExtFormFormPanel)
  private
  ...
  // A send QR panel, suitable for embedding into HTML code.
  // requires the ContainerElementId config property to be set,
  // otherwise it is not displayed.
  TKExtSendQRPanel = class(TKExtPanelControllerBase)
  strict protected
...
procedure TKExtSendQRPanel.DoDisplay;
var
  LDummyHeight: Integer;
  LFormPanel: TKExtSendQRFormPanel;
...
begin
  ...
  begin
    ...
    LFormPanel := TKExtSendQRFormPanel.CreateAndAddTo(LBorderPanel.Items);
    LFormPanel.Region := rgCenter;
  end
  else
    LFormPanel := TKExtSendQRFormPanel.CreateAndAddTo(Items);
...
{ TKExtSendQRFormPanel }

function TKExtSendQRFormPanel.GetEnableButtonJS: string;
begin
  Result := Format(
...
function TKExtSendQRFormPanel.GetSubmitJS: string;
begin
  Result := Format(
...
procedure TKExtSendQRFormPanel.Display(const AEditWidth: Integer; const AConfig: TEFNode;
...
begin
  ...
  FStatusBar.BusyText := _('Generating QR code...');
  Bbar := FStatusBar;
...
procedure TKExtSendQRFormPanel.InitDefaults;
begin
  inherited;
  LabelAlign := laRight;
  Border := False;
...
procedure TKExtSendQRFormPanel.DoSend;
var
  LParams: TEFNode;
begin
  ...
  try
    ...
    try
      Session.Config.Authenticator.QRGenerate(LParams);
      Session.ShowMessage(emtInfo, _('Send QR code'), _('The QR code for your account was generated and sent to the specified e-mail address.'));
...
initialization
  TKExtControllerRegistry.Instance.RegisterClass('SendQR', TKExtSendQRPanel);

finalization
  TKExtControllerRegistry.Instance.UnregisterClass('SendQR');
