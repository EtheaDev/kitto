...
uses
  SysUtils,Variants,
   EF.Localization, EF.Logger, Kitto.Auth, ExtPascal,
  Kitto.Metadata.DataView, Auth,
...
begin
  inherited;
  if not Session.Config.Authenticator.IsAuthenticated then
  begin
    Session.ShowMessage(emtInfo, _('New User'), _('An e-mail with login instructions has been sent'));
  end;
end;
...
begin
  ...
  begin
    ...
    if TKConfig.Instance.Config.GetBoolean('IsClearPassword') then
      ARecord.FieldByName('PASSWORD_HASH').AsString := LPassword
    else if ContainsText(Session.Config.Authenticator.GetClassId,'BCrypt') then
